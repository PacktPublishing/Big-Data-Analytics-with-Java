...
public class ZipCntBarChart extends ApplicationFrame
{
	
	public static String APP_NAME = "GradeCntBarChart";
	public static String APP_MASTER = "local";
	
   public ZipCntBarChart( String applicationTitle , String chartTitle )
   {
...
//   }
   ...
   private CategoryDataset createDataset() {
		...
		//	System.out.println("loanId --> " + loanId);
			...
			String zipCode = r.get(23).toString();
			...
			if(null == annualInc || "".equals(annualInc) || 
				...
				null == loanStatus || "".equals(loanStatus) ||
				null == zipCode || "".equals(zipCode)) return null;
	    ...
	    Dataset<Row> dataDS = spark.createDataFrame(data.rdd(), LoanVO.class);
			 dataDS.createOrReplaceTempView("loans");
	    			 ...
	    			 Dataset<Row> loanAmtDS = spark.sql("select zipCode,count(*) loanCount from loans where loanStatus = 0.0 group by zipCode order by loanCount desc limit 10");
	    loanAmtDS.show();
	    final DefaultCategoryDataset dataset = 
	    	      new DefaultCategoryDataset( );  	    
   ...
   {
	  ZipCntBarChart chart = new ZipCntBarChart("Zip code with max defaulted loans", "Top ten zipcodes with max defaulted loans.");
      chart.pack( );        
